...
class CleanupAliasTest {
    ...
    @Test
    fun deleteKeyIfPresentRemovesTheAliasUsedForGeneration() {
    ...
    @Test
    fun deleteKeyIfPresentIsANoOpForAnAliasThatIsAlreadyGone() {
        val credId = Fido2Util.generateRandomByteArray(32).toBase64url()
        assertThat(SecureExecutionHelper.containAlias(credId)).isFalse()

        // Must not throw: cleanup runs on a failure path and `retryCleanup` calls it more than once, so a
        // second pass over an already-deleted alias has to be a success.
        SecureExecutionHelper.deleteKeyIfPresent(credId)
