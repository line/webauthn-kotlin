 ...
 *
 * `android:exported="true"` on this activity was an intent-redirection and authentication-bypass hole:
 * any installed app could start it, and it used to launch an `Intent` taken from its own extras under
 * the host app's identity. Android lint does not flag an exported activity - being exported is a
 * legitimate choice for most - so nothing else in the build fails if the attribute comes back, which is
